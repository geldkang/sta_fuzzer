                    ...
                    while 1:
                        ...
                        if self.proto == socket.SOCK_STREAM or self.proto == socket.SOCK_DGRAM:
                            ...
                            try:
                                ...
                                sock = conf.L2socket(type=ETH_P_ALL, iface="wlx64e599fa39fc")
                        ...
                        try:
                            for e in path[:-1]:
                                ...
                                self.transmit(sock, node, e, target) # error function
                        except Exception, e:
    ...
    def transmit (self, sock, node, edge, target):
        ...
        try:
            if self.proto == socket.SOCK_STREAM:
                ...
                #pckt = RadioTap() / Dot11(type=0, subtype=0, addr1='90:00:db:aa:c3:e9',addr2="64:e5:99:fa:39:fc",addr3="64:e5:99:fa:39:fc")/IP()/Dot11AssoReq()/data
                #pckt = RadioTap() / Dot11(type=0, subtype=10, addr1='90:00:db:aa:c3:e9',addr2="64:e5:99:fa:39:fc",addr3="64:e5:99:fa:39:fc")/Dot11Disas()/data
                data = random_char(10)
                pckt = RadioTap()/Dot11(
                    proto=0, FCfield=0, subtype=5, addr4=None, addr2='64:e5:99:2c:5c:d2', addr3='64:e5:99:2c:5c:d2', addr1='90:00:db:aa:c3:e9', SC=57840, type=0, ID=14849)/Dot11ProbeResp(
                        timestamp=8371943815456, cap=4356, beacon_interval=100)/Dot11Elt(
                            info='fffffffff', ID=0, len=9)/data
                wrpcap('presponse.pcap',pckt,append=True)

                sendp(pckt, iface='wlx64e599fa39fc', socket=sock)
                #self.last_recv = srp1(pckt, iface='wlx64e599fa39fc', filter='ether host 90:00:db:aa:c3:e9')
                #sock.sendto(data, (self.targets[0].host, self.targets[0].port)) # error point
